...
public class CookieTestServlet extends HttpServlet {
    public void doGet(HttpServletRequest req, HttpServletResponse res)
        ...
        if ((cookies != null) && (cookies.length > 0)) {
            out.println("추출된 쿠키 정보<br><table border=1>");

            for (int i = 0; i < cookies.length; i++) {
                Cookie cookie = cookies[i];

                out.print("<tr><td>쿠키 이름: </td><td>" + cookie.getName() +
                    "</td>");
                out.print("<td>쿠키 값: </td><td>" + cookie.getValue() +
                    "</td></tr>");
            }
            out.print("</table>");
        }
        ...
        out.print("<form method=post action=/servlet.CookieTestServlet><br>");
        out.println("쿠키 이름: " +
            "<input type=text length=20 name=cookiename><br>");
        out.println("쿠키 값: " +
            "<input type=text length=20 name=cookievalue><br></form>");
        out.println("유효시간: " +
            "<input type=text length=20 name=expiretime>(시간단위)<br>");
